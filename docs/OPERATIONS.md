...
`PAGELOOM_BACKUP_BUCKET` defaults to the dedicated, versioned `pageloom-os-production-backups` bucket. `dailyFirestoreBackup` runs as the dedicated `pageloom-backup@pageloom-os-production.iam.gserviceaccount.com` identity; grant only that identity `roles/datastore.importExportAdmin`, and grant the Firestore service agent bucket-scoped access required for managed exports. The bucket uses uniform access, enforced public-access prevention, an unlocked 30-day retention period, and lifecycle deletion after 90 days. The job starts a managed Firestore export at 02:30 Asia/Jerusalem and records the long-running operation under `systemOperations/backups/runs`. Missing configuration and failed export requests are recorded explicitly; they never report a false success.

`backupFreshnessWatchdog` (every 6 hours) checks Firestore backup freshness, media/storage transfer freshness, and Google Cloud Service Health incidents. Every warning or error it finds is written to Firestore (`systemOperations/watchdog/alerts`), not only to Cloud Logging, and each run also updates `systemOperations/watchdogHeartbeat` so a stopped schedule is itself detectable. The authenticated `GET /api/platform/system-health` endpoint (platform administrators only) returns the latest backup runs, the recent watchdog alerts, and the current heartbeat in one place, so checking system health does not require reading logs.
