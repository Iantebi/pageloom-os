...
**Historical reliability note (2026-08-30 investigation)**: Cloud Scheduler was observed retrying `dailyFirestoreBackup` and `monitorBusinessRisks` far more aggressively than their configured `retryCount:3`, producing misleading error-level log entries even on days the backup had already succeeded. Root-caused and fixed and deployed: `monitorBusinessRisks`, `monitorWorkflowTimeouts`, and `dailyCeoReport` were silently exceeding their default 256MiB memory allocation (all bumped to 512MiB); `dailyFirestoreBackup` now records a duplicate "already exists" retry as `already_completed` instead of throwing a failure. Backup data integrity was never actually at risk — every affected day still has a complete, valid export.

**How failures are detected (implemented 2026-08-30/31)**: 10 Cloud Monitoring alert policies (P1–P10) cover Firestore backup failure, Storage backup failure, Scheduler failure, repeated/sustained Function errors, memory exhaustion, permission/access failures, production 5xx and availability, and a dedicated `backupFreshnessWatchdog` function (runs every 6h) that independently re-verifies Firestore and Storage Transfer backup freshness plus relevant Google Cloud/Firebase Service Health incidents — with its own native dead-man alert if the watchdog itself stops running. All route to a single email channel (`pageloom.studio@gmail.com`); Essential Contacts is configured for both `pageloom.studio@gmail.com` and `iantebi5@gmail.com`. One real bug was found and fixed in the watchdog itself: its Firestore freshness check originally anchored to midnight UTC of the backup folder's date label rather than the actual completion time, causing a false CRITICAL alert on 2026-08-31 (the daily export runs at 23:30 UTC during Israeli DST, ~23.5h before that folder-date's midnight) — fixed to use the real object timestamp instead, verified against production, and covered by a regression test.
