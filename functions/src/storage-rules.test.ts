...
describe("Storage tenant isolation policy", () => {
  ...
  it("scopes Business Discovery uploads by project (2026-09-16: client writes use the tighter, feature-specific safeDiscoveryUpload ceiling; staff keep the broader safeUploadShape())", () => {
    expect(rules).toContain("match /organizations/{orgId}/discovery/{projectId}/{sectionId}/{fieldId}/{userId}/{allPaths=**} {");
    expect(rules).toContain("allow read: if staff(orgId) || clientProject(orgId,projectId);");
    expect(rules).toContain("allow write: if staff(orgId) ? safeUploadShape() : (safeDiscoveryUpload(orgId,userId) && clientProject(orgId,projectId));");
    expect(rules).toContain("function safeDiscoveryUpload(orgId, userId)");
  });
