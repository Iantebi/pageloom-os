...
describe("AI-native core", () => {
  ...
  it("supports provider-independent API routing",()=>{const base={agent:agents[0]!,priority:"critical" as const,requiresCode:true,requiresGoogleContext:false,budgetRemainingUsd:100};expect(routeModel({...base,preferredProvider:"openai" as const,providerHealth:{gemini:true,openai:true}})).toMatchObject({provider:"openai"});expect(routeModel({...base,preferredProvider:"openai" as const,providerHealth:{gemini:true,openai:false}})).toMatchObject({provider:"gemini"});expect(()=>routeModel({...base,providerHealth:{gemini:false,openai:false}})).toThrow(/enabled and healthy/) });
