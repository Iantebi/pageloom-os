...
export function missingRequiredQuestionnaireFields(fields:z.infer<typeof questionnaireFieldSchema>[],responses:z.infer<typeof questionnaireResponseSchema>,filePaths:string[]):string[]{return fields.filter(field=>{if(!field.required)return false;if(field.type==="file")return !filePaths.some(path=>path.includes(`/${field.id}/`));const value=responses[field.id];return value===undefined||value===false||value===""||(Array.isArray(value)&&value.length===0)}).map(field=>field.id)}
export function validateRequiredQuestionnaireFields(fields:z.infer<typeof questionnaireFieldSchema>[],responses:z.infer<typeof questionnaireResponseSchema>,filePaths:string[]){const missing=missingRequiredQuestionnaireFields(fields,responses,filePaths);if(missing.length)throw new Error(`Missing required questionnaire fields: ${missing.join(", ")}`)}
