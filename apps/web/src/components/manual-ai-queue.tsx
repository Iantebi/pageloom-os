...
export function ManualAiQueue(){const{organizationId}=useOrganization();const tasks=useLiveCollection<Task>(organizationId?`organizations/${organizationId}/tasks`:undefined,"updatedAt");const queue=useMemo(()=>tasks.data.filter(task=>task.status==="awaiting_manual_ai"&&task.manualAi).reverse(),[tasks.data]);const[selectedId,setSelectedId]=useState("");const selected=queue.find(task=>task.id===(selectedId||queue[0]?.id));const s=t("manualAiQueue");return <Card><CardHeader icon={Bot} title={s.title} subtitle={s.subtitle} action={selected?<Status value="awaiting_manual_ai" label={s.awaitingManualAi}/>:undefined}/>{!selected?<Empty title={s.emptyTitle} description={s.emptyDescription}/>:<div className="grid gap-5 xl:grid-cols-[260px_1fr]" dir={selected.locale==="he"?"rtl":"ltr"}><nav className="space-y-2">{queue.map(task=><button key={task.id} onClick={()=>setSelectedId(task.id)} className={`w-full rounded-xl border p-3 text-start ${task.id===selected.id?"border-[#7357ff] bg-[#f4f1ff]":"border-[var(--border)]"}`}><b className="block text-[10px] capitalize">{task.context.workflowStage?s.stageLabel(String(task.context.workflowStage)):task.agentId}</b><span className="mt-1 block text-[9px] text-[var(--muted)]">{task.agentId}</span></button>)}</nav><ManualTask task={selected} organizationId={organizationId}/></div>}</Card>}
