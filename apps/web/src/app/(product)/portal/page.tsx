...
export default function Portal() {
  ...
  return <div className="space-y-6">
    ...
    <Card><div className="grid gap-3 md:grid-cols-4">{s.steps.map((step,index)=><div className="rounded-xl bg-[#fafaf8] p-4" key={step}><b className="text-xs">{step}</b><p className="mt-2 text-[10px] leading-4 text-[var(--muted)]">{index<2?s.stepActionNeeded:s.stepAutoUpdates}</p></div>)}</div></Card>
    {projects.error && <Card role="alert"><p className="text-xs text-red-700">{s.projectsError}</p></Card>}
    {projects.loading && !projects.data.length ? <Card aria-busy="true"><div className="flex min-h-40 items-center justify-center gap-2 text-xs text-[var(--muted)]"><LoaderCircle className="h-4 w-4 animate-spin" />{s.loadingProject}</div></Card> : project ? <>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card className="lg:col-span-2"><h2 className="text-sm font-semibold">{s.commentsTitle}</h2><label className="field mt-4"><span>{s.commentLabel}</span><textarea className="input min-h-28" value={comment} onChange={event => setComment(event.target.value)} placeholder={s.commentPlaceholder} maxLength={4000} /></label><div className="mt-3 flex flex-wrap gap-2"><Button variant="secondary" disabled={!comment.trim() || busy} onClick={() => void send()}><Send className="h-4 w-4" />{s.sendComment}</Button>{project.workflowStage === "customer_review" && <><Button variant="secondary" disabled={busy} onClick={() => void review("CustomerRequestedRevision")}><RotateCcw className="h-4 w-4" />{s.requestChanges}</Button><Button disabled={busy} onClick={() => void review("CustomerApproved")}><CheckCircle2 className="h-4 w-4" />{s.approveSite}</Button></>}</div>{message && <p className="mt-4 rounded-lg bg-green-50 p-3 text-xs text-green-800" role="status">{message}</p>}{error && <p className="mt-4 rounded-lg bg-red-50 p-3 text-xs text-red-700" role="alert">{error}</p>}</Card>
...
function CustomerQuestionnaire({ organizationId, projectId, questionnaires }: { organizationId: string; projectId: string; questionnaires: ReturnType<typeof useLiveCollection<Questionnaire>> }) {
  ...
  if (questionnaires.error) return <Card className="lg:col-span-2" role="alert"><p className="text-xs text-red-700">{s.error}</p></Card>;
...
function QuestionnaireForm({ organizationId, projectId, questionnaire }: { organizationId: string; projectId: string; questionnaire: Questionnaire }) {
  ...
  return <Card className="lg:col-span-2"><form onSubmit={submit}><div className="flex flex-wrap items-start justify-between gap-3"><div><span className="eyebrow">{s.versionEyebrow(questionnaire.version)}</span><h2 className="mt-2 flex items-center gap-2 text-lg font-semibold"><FileText className="h-5 w-5" />{questionnaire.title}</h2><p className="mt-2 text-xs leading-5 text-[var(--muted)]">{s.intro}</p></div><Status value={questionnaire.status} /></div><fieldset disabled={completed || saving} className="mt-6 space-y-5 disabled:opacity-70">{questionnaire.fields.map(field => <label className="field" key={field.id}><span>{field.label}{field.required && " *"}</span>{field.helpText && <small className="text-[10px] leading-4 text-[var(--muted)]">{field.helpText}</small>}<QuestionnaireInput field={field} value={responses[field.id]} fileReady={paths.some(path => path.includes(`/${field.id}/`))} uploading={uploading === field.id} setValue={value => setResponses(current => ({ ...current, [field.id]: value }))} upload={file => void upload(field, file)} /></label>)}</fieldset>{!completed && <Button className="mt-6" disabled={saving || Boolean(uploading)} type="submit">{saving ? s.submitting : s.submit}</Button>}{completed && <p className="mt-6 flex items-center gap-2 rounded-lg bg-green-50 p-3 text-xs text-green-800"><CheckCircle2 className="h-4 w-4" />{s.completed}</p>}{message && <p className="mt-4 rounded-lg bg-green-50 p-3 text-xs text-green-800" role="status">{message}</p>}{error && <p className="mt-4 rounded-lg bg-red-50 p-3 text-xs text-red-700" role="alert">{error}</p>}</form></Card>;
}
...
function QuestionnaireInput({ field, value, fileReady, uploading, setValue, upload }: { field: QuestionnaireField; value: string | boolean | string[] | undefined; fileReady: boolean; uploading: boolean; setValue: (value: string | boolean | string[]) => void; upload: (file?: File) => void }) {
  ...
  if (field.type === "file") return <><input required={field.required && !fileReady} type="file" className="input" onChange={event => upload(event.target.files?.[0])} />{uploading && <small className="flex items-center gap-1 text-[10px] text-[var(--muted)]"><LoaderCircle className="h-3 w-3 animate-spin" />{s.uploadingFile}</small>}{fileReady && !uploading && <small className="text-[10px] text-green-700">{s.fileSaved}</small>}</>;
